//! Dilithium (ml_dsa_65) keypair implementation
...
impl Keypair {
    ...
    pub fn from_phrase(mnemonic: &bip39::Mnemonic, password: Option<&str>) -> Result<Self, Error> {
        ...
        Ok(Self(ml_dsa_65::Keypair::generate(Some(&seed[..]))))
    }

    /// Create a keypair from a 32-byte seed.
    pub fn from_seed(seed: SecretKeyBytes) -> Self {
        Self(ml_dsa_65::Keypair::generate(Some(&seed[..])))
    }
    ...
    pub fn sign(&self, message: &[u8]) -> SignatureBundle {
        let sig = self
            ...
            .expect("ml_dsa_65 signing returned None");
...
pub fn verify<M: AsRef<[u8]>>(sig: &SignatureBundle, message: M) -> bool {
    // qp 1.0.1 ml_dsa_65: from_bytes returns PublicKey directly.
    let pk = ml_dsa_65::PublicKey::from_bytes(&sig.public);
